...
func (a *Nrqldroprules) NRQLDropRulesCreateWithContext(
	...
	if len(resp.NRQLDropRulesCreateResult.Errors) > 0 {
		errs := fmt.Errorf("query error")
		for _, err := range resp.NRQLDropRulesCreateResult.Errors {
...
func (a *Nrqldroprules) NRQLDropRulesDeleteWithContext(
	...
	// If we got errors back, wrap them all up
	if len(resp.NRQLDropRulesDeleteResult.Errors) > 0 {
		errs := fmt.Errorf("query error")
		for _, err := range resp.NRQLDropRulesDeleteResult.Errors {
			errs = fmt.Errorf("%w; %s", errs, err.Description)
		}
		return nil, errs
	}
